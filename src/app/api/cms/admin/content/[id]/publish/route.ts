...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (!userCanPublishCmsContentType(user, existing.contentType)) {
    return cmsJson({ error: 'CMS role cannot publish this content type.' }, { status: 403 });
  }
  if (!userCanPublishCmsContent(user, existing)) {
    return cmsJson({ error: 'CMS role cannot publish this content.' }, { status: 403 });
  }
  const content = await store.publishContent(id, user.id);
